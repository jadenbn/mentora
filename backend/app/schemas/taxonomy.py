"""Provider output for skill identification, before build_taxonomy runs.

Shape-only validation lives here. The rules that make a taxonomy usable --
bounded skill count, bounded keyword/question-form lists -- are enforced
once, in app.services.taxonomy.build_taxonomy, the same path a hand-authored
course JSON takes.
"""
...
# Catching this here means a violation gets the workflow's own repair retry,
# with the real error fed back, instead of surfacing as a hard failure
# downstream in build_taxonomy with no chance to fix it.
_MAX_ENTRY_CHARS = 80
...
class TaxonomyPlan(StrictModel):
    """A batch of skills, e.g. pasted through the dev import endpoint."""
