...
from app.services import attribution, selection
...
def _resolve_skills(session: Session, payload: AttemptCreate) -> list[str]:
    ...
    When the problem was generated by us, its ProblemSkill rows are the source
    of truth and the client's expected_skills is only a hint we cross-check --
    a client cannot attribute an attempt to a skill it names. Problems with no
    row (legacy, or externally authored) fall back to the client's list, and
    that fallback is logged rather than silent.
    """
